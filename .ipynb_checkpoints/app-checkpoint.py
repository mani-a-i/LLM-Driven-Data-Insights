...
question = st.text_input(label = 'Enter your prompt')
text = prompt.format(question)

button = st.button('generate')
if button:
    if prompt:
        out = gen(text).replace("```","")
        st.code(out)
        print('*'*100)     
        
        if out.find('plt.show()')!=-1:
            out_modified = out + 'st.pyplot(plt.gcf()) '
            exec(out_modified)
        elif out.find('fig.show()')!=-1:
            out_modified = out + 'st.plotly_chart(fig) '
            exec(out_modified)
        else:   
        ...
        st.write('Looks like no input was given')
